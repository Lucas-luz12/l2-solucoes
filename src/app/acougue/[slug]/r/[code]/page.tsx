...
import { ShopImage } from "@/components/acougue/ShopImage";
...
export default async function ReservationPage({
  ...
  let result;
  try {
    result = await getReservationByCode(slug, code);
  } catch (error) {
    ...
    <div className="min-h-screen bg-surface px-4 py-12">
      <article className="mx-auto max-w-xl rounded-md border border-line bg-white px-6 py-8">
        <div className="flex items-center gap-3">
          {shop.logoUrl ? <ShopImage src={shop.logoUrl} alt="" className="h-12 w-12 rounded-md object-cover" /> : null}
          <p className="font-display text-sm font-semibold uppercase tracking-[0.18em] text-accent">{shop.name}</p>
        </div>
